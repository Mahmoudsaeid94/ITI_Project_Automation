...
// to Download and manage Allure binaries, ensuring the correct version is used and available for report generation

public class AllureBinaryManager {

    private static  class LazyHolder
    {
        static final String VERSION = resolveVersion(); //2.42.0
    ...
    public static void downloadAndExtract() {
        try {
            ...
            //C:\Users\Mhmoud\.m2\repository\allure\allure-2.42.0
            if (Files.exists(extractionDir)) {
                LogsManager.info("Allure binaries already exist and same version.");
                return;
            }
    ...
    public static Path getExecutable() {
        String version = LazyHolder.VERSION;
        //C:\Users\Mhmoud\.m2\repository\allure\allure-2.41.0\bin
    ...
    private static Path downloadZip(String version)   {
        try {
            //https://repo.maven.apache.org/maven2/io/qameta/allure/allure-commandline/2.34.1/allure-commandline-*.**.*.zip
            String url = AllureConstants.ALLURE_ZIP_BASE_URL + version + "/allure-commandline-" + version + ".zip";
            //C:\Users\Mhmoud\.m2\repository\allure
